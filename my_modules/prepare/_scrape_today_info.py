# 2023/02/05 ubuntusで標準時がUTCとなってしまうとこを修正
import datetime
from datetime import timezone,timedelta
import re
from selenium.webdriver.common.by import By
from my_modules.constants import UrlPaths
...
def create_active_race_id_list(minus_time=-50):
    ...
    """
    # 現在時刻を取得
    now_date = datetime.datetime.now(timezone(timedelta(hours=+9))).date().strftime('%Y%m%d')
    hhmm = datetime.datetime.now(timezone(timedelta(hours=+9))).strftime("%H:%M")
    print(now_date, hhmm)
